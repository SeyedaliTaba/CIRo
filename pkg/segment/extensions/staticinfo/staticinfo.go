...
type CarbonInfo struct {
	Intra map[common.IFIDType][]uint8
}
...
func FromPB(pb *cppb.StaticInfoExtension) *Extension {
	...
	return &Extension{
		Carbon:       carbonInfoFromPB(pb.Carbon),
...
func carbonInfoFromPB(pb *cppb.CarbonInfo) CarbonInfo {
	if pb == nil || len(pb.Intra) == 0 {
		return CarbonInfo{}
	}
	intra := make(map[common.IFIDType][]uint8, len(pb.Intra))
	for ifid, v := range pb.Intra {
		intra[common.IFIDType(ifid)] = make([]uint8, len(v))
		for i := 0; i < len(v); i++ {
			intra[common.IFIDType(ifid)][i] = v[i]
		}
	}
	return CarbonInfo{
		Intra: intra,
...
func ToPB(si *Extension) *cppb.StaticInfoExtension {
	...
	return &cppb.StaticInfoExtension{
		Carbon:       carbonInfoToPB(si.Carbon),
...
func carbonInfoToPB(ci CarbonInfo) *cppb.CarbonInfo {
	if len(ci.Intra) == 0 {
		return nil
	}
	intra := make(map[uint64][]byte, len(ci.Intra))
	for ifid, v := range ci.Intra {
		intra[uint64(ifid)] = make([]byte, len(v))
		for i := 0; i < len(v); i++ {
			intra[uint64(ifid)][i] = v[i]
		}
	}
	return &cppb.CarbonInfo{
		Intra: intra,
	}
}
