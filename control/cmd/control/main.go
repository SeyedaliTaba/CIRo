...
func realMain(ctx context.Context) error {
	...
	tasks, err := cs.StartTasks(cs.TasksConfig{
		...
		StaticInfo: func() *beaconing.StaticInfoCfg {
			staticInfo, err := beaconing.ParseStaticInfoCfg(globalCfg.General.StaticInfoConfig())
			log.Info("sataticInfo run at: " + time.Now().String())
			if err != nil {
				log.Info("No static info file found. Static info settings disabled.", "err", err)
			}
			return staticInfo
		...
		CarbonForecastFile:        globalCfg.General.StaticInfoConfig(),
		GeolocationFile:           globalCfg.General.Geolocation(),
		ElectricityMapsAPIKeyFile: globalCfg.General.ElectricityMapsAPIKeyConfig(),
		...
		CarbonForecastInterval:    globalCfg.BS.CarbonForecastInterval.Duration,
