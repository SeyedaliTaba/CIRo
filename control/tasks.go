...
type TasksConfig struct {
	...
	CarbonForecastInterval time.Duration
...
// CarbonForecaster starts a periodic task to forecast the carbon intensity between interface pairs of an AS
func (t *TasksConfig) CarbonForecaster() *periodic.Runner {
	s := &beaconing.CarbonForecaster{
		ElectricityMapsAPIKeyFile: t.ElectricityMapsAPIKeyFile,
		GeolocationFile:           t.GeolocationFile,
		CarbonForecastFile:        t.CarbonForecastFile,
		AllInterfaces:             t.AllInterfaces,
		Tick:                      beaconing.NewTick(t.CarbonForecastInterval),
		Success:                   false,
		ForecastAvailable:         true,
	}
	return periodic.Start(s, 500*time.Millisecond, t.CarbonForecastInterval)
}
...
type Tasks struct {
	CarbonForecaster *periodic.Runner
...
func StartTasks(cfg TasksConfig) (*Tasks, error) {
	...
	return &Tasks{
		CarbonForecaster: cfg.CarbonForecaster(),
