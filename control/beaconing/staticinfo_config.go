...
type StaticInfoCfg struct {
	Carbon    map[common.IFIDType]InterfaceCarbon     `json:"Carbon"`
...
func (cfg StaticInfoCfg) Generate(intfs *ifstate.Interfaces,
	ingress, egress uint16, ts time.Time) *staticinfo.Extension {

	ifType := interfaceTypeTable(intfs)
	return cfg.generate(ifType, common.IFIDType(ingress), common.IFIDType(egress), ts)
}

func (cfg StaticInfoCfg) generate(ifType map[common.IFIDType]topology.LinkType,
	ingress, egress common.IFIDType, ts time.Time) *staticinfo.Extension {

	return &staticinfo.Extension{
		Carbon:       cfg.generateCarbon(ifType, ingress, egress, ts),
...
func (cfg StaticInfoCfg) generateCarbon(ifType map[common.IFIDType]topology.LinkType,
	ingress, egress common.IFIDType, ts time.Time) staticinfo.CarbonInfo {
	c := staticinfo.CarbonInfo{
		Intra: make(map[common.IFIDType][]uint8),
	}

	for ifid, v := range cfg.Carbon[egress].Intra {
		if includeIntraInfo(ifType, ifid, ingress, egress) {
			c.Intra[ifid] = make([]uint8, 48)
			hourDifference := time.Now().Hour() - ts.Hour()
			...
			for i := hourDifference; i < 24; i++ {
				c.Intra[ifid][i] = v[i-hourDifference]
				c.Intra[ifid][i+24] = v[i-hourDifference+24]
			}
		}
	}
