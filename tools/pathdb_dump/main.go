...
type asCarbon struct {
	IA     addr.IA
	carbon map[common.IFIDType][]uint8
}
...
func newSegment(res *query.Result) (segment, error) {
	...
	carbons := make([]asCarbon, 0, len(res.Seg.ASEntries))
	for _, ase := range res.Seg.ASEntries {

		hop := ase.HopEntry.HopField

		if ase.Extensions.StaticInfo != nil && ase.Extensions.StaticInfo.Carbon.Intra != nil && (hop.ConsIngress > 0 || hop.ConsEgress > 0) {
			carbon := asCarbon{
				IA:     ase.Local,
				carbon: ase.Extensions.StaticInfo.Carbon.Intra,
			}
			carbons = append(carbons, carbon)
		}
...
func (s segment) toString(showTimestamps bool) string {
	str := fmt.Sprintf("%s %4s %s %s", s.LoggingID, s.SegType, ifsArrayToString(s.interfaces), carbonToString(s.Carbon))
	if showTimestamps {
		now := time.Now()
...
func carbonToString(carbons []asCarbon) string {
	if len(carbons) == 0 {
		return ""
	}
	strs := []string{}
	strs = append(strs, "\n")
	for i := 0; i < len(carbons); i++ {
		strs = append(strs, fmt.Sprintf("###################### %s ##################\n", carbons[i].IA))
		for ifid, val := range carbons[i].carbon {
			strs = append(strs, fmt.Sprintf("%d: ", ifid))
			for _, v := range val {
